...
wire1Dirs=inVal1.split(",")
wire2Dirs=inVal2.split(",")
...
x=0
y=0
for direc in wire1Dirs:
    direction=direc[0]
    magnitude=int(direc[1:])
    if(direction=="R"):
        print(x,x+magnitude,y,y)
        w1h=w1h+[[y,x,x+magnitude]]
        x=x+magnitude
    if(direction=="L"):
        print(x-magnitude,x,y,y)
        w1h=w1h+[[y,x-magnitude,x]]
        x=x-magnitude
    if(direction=="U"):
        print(x,x,y,y+magnitude)
        w1v=w1h+[[x,y,y+magnitude]]
        y=y+magnitude
    if(direction=="D"):
        print(x,x,y-magnitude,y)
        w1v=w1h+[[x,y-magnitude,y]]
        y=y-magnitude
...
minDistCross=1e1000
crossing=[]
for direc in wire2Dirs:
    direction=direc[0]
    magnitude=int(direc[1:])
    if(direction=="R"):
        print(x,x+magnitude,y,y)
        for vert in w1v:
            if(vert[0]<x+magnitude and vert[0]>x and y>vert[1] and y<vert[2] and not (x==0 and y==0)):
                tmp=abs(vert[0])+abs(y)
                if(tmp<minDistCross):
                    minDistCross=tmp
                    crossing=[vert[0],y]
                crossings=crossings+[[vert[0],y]]
        x=x+magnitude
    if(direction=="L"):
        print(x,x-magnitude,y,y)
        for vert in w1v:
    ...
    if(direction=="U"):
        print(x,x,y,y+magnitude)
        for hori in w1h:
            if(hori[0]<y+magnitude and hori[0]>y and x>hori[1] and x<hori[2] and not (x==0 and y==0)):
                tmp=abs(hori[0])+abs(x)
                if(tmp<minDistCross):
                    minDistCross=tmp
                    crossing=[x,hori[0]]
                crossings=crossings+[[x,hori[0]]]
        y=y+magnitude
    if(direction=="D"):
        print(x,x,y,y-magnitude)
        for hori in w1h:
